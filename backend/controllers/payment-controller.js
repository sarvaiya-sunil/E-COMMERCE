...
export const createCheckoutSession = async (req, res) => {
  try {
    ...
    const lineItems = products.map((product) => {
      const amount = Math.round(product.price * 100); // -> it will convert the dollar in the form of cents
...
async function createNewCoupon(userId) {
  ...
  const newCoupon = new Coupon({
    ...
    expirationDate: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000), // 30 days from current day
    userId: userId,
  });
